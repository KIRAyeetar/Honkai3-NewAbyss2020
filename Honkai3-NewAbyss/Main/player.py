...
class Player(object):
    ...
    # cup:奖杯数
    # cup_virtual:虚拟奖杯数
    def __init__(self, fund_level, cup=0, cup_virtual=0, player_id='None', flag_abyss_8=0):
        # 最初深渊等级
        self.fund_level = fund_level
        # 最新深渊等级(1-9)
        self.abyss_level = fund_level
        self.cup = cup
    ...
    # 引入越底层越容易不打机制，(0~氪金水平)随机数 小于 (0~n)随机数 时，effort_level直接为0；n默认为3,约40%的人不打; 范围[0, 13.5]
    ...
    # 水晶收益增加;
    def add_crystal(self, num):
        self.crystal += num

    # 更新深渊最新等级;
    def update_abyss_level(self, num):
        self.abyss_level = num

    # 第一次进入红莲的虚拟奖杯判断
    def is_first_abyss_8(self):
        if self.flag_abyss_8 == 0:
